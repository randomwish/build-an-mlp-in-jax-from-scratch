...
def softmax_probabilities(logits):
    ...
    new_logits = logits - jnp.max(logits)
    exp_logits = jnp.exp(new_logits)
    sum_logits = jnp.sum(exp_logits, axis=-1, keepdims=True)
...
def mlp_forward(params, x):
    ...
    for layer in params[:-1]:
        h = relu_activation(linear_forward(h,layer))
    
    logits = linear_forward(h, params[-1])
    return logits
...
def log_softmax_logits(logits):
    ...
    max_row = jnp.max(logits, axis = -1,keepdims=True)
    shifted = logits - max_row
    return logits - max_row - jnp.log(jnp.sum(jnp.exp(shifted),axis = -1,keepdims=True))
...
def cross_entropy_loss(logits, one_hot_targets):
    ...
    mean_loss = -(one_hot_targets * log_probs).sum(axis=-1).mean()
    return mean_loss
...
def classification_accuracy(logits, labels):
    ...
    arg_correct = jnp.argmax(logits,axis=-1)
    correct = jnp.equal(arg_correct, labels)
...
def training_step(params, x, one_hot_targets, learning_rate):
    ...
    current_loss = loss_fn_of_params(params, x, one_hot_targets)
...
def train_mlp(params, x, one_hot_targets, learning_rate, num_epochs):
    ...
    for idx in range(num_epochs):
        new_params, _ = training_step(new_params, x, one_hot_targets, learning_rate)
    return new_params
